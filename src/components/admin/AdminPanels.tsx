...
function statusTone(value: string | null | undefined) {
  if (value === "completed" || value === "paid" || value === "active") return "border-primary/20 bg-primary/10 text-primary";
  if (value === "pending" || value === "pending_confirmation" || value === "needs_review") return "border-accent/20 bg-accent/10 text-accent";
...
export function PaymentsPanel(props: {
  ...
  orderReviewForm: { orderCode: string; amount: string; txCode: string; note: string };
  onOrderReviewFormChange: (patch: Partial<{ orderCode: string; amount: string; txCode: string; note: string }>) => void;
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
      <div className={`${SURFACE} space-y-4`}>
        <div>
          <div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-primary">Finance console</div>
          <h3 className="mt-2 text-xl font-semibold">Payments và entitlement</h3>
          <p className="mt-2 text-sm leading-6 text-zinc-600">Finance được quyền log giao dịch thủ công, mark paid cho order CT và rà soát anomalies để user vào Telegram được ngay.</p>
        </div>
        ...
        <div>
          <div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-primary">Reconciliation</div>
          <h4 className="mt-2 text-lg font-semibold">Mark paid theo order code</h4>
          <p className="mt-2 text-sm leading-6 text-zinc-600">Dùng cho case chuyển khoản đúng mã CT nhưng webhook chưa grant entitlement.</p>
        </div>
