...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const result = await estimateZaloNutrition(req, body);
    sendJson(res, 200, result);
  } catch (error) {
    const message = String((error as Error)?.message || error || "zalo_nutrition_estimate_failed");
