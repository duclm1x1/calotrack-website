...
import { normalizeVietnamPhoneInput } from "./portalPhoneAuthServer.js";
...
async function resetPhoneOnboardingFixture(admin: any, phoneInput: unknown) {
  const phoneE164 = normalizeVietnamPhoneInput(String(phoneInput || "")) || "";
  if (!phoneE164) {
...
export async function handleAdminIdentitiesRequest(req: any, res: any) {
  ...
  try {
    ...
    if (action === "sync_compat_user_to_customer") {
      ...
      await admin
        ...
        .eq("linked_user_id", compatRow.id);

      if (safeString(compatRow.auth_user_id)) {
        const existingAuthLink =
          (await maybeSingle<Record<string, any>>(
            admin
              .from("customer_auth_links")
              .select("*")
              .eq("auth_user_id", compatRow.auth_user_id)
              .limit(1),
          )) || null;
        ...
        if (!existingAuthLink) {
          await admin.from("customer_auth_links").insert({
            customer_id: customer.id,
            auth_user_id: compatRow.auth_user_id,
            email: safeString(compatRow.email),
            link_status: "linked",
          });
        } else {
          await admin
